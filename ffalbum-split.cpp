...
class MusicAlbum
{
    ...
    m_chart(chart)
    {
    if(!boost::filesystem::exists(boost::filesystem::path(m_filename))) throw std::runtime_error("Your album audio file seems not to exist!");
    parse_from_file();
    match_end_times();
    }
...
void MusicAlbum::parse_from_file()
{
    ...
    try{
    ifs.open(m_chart);
    if(!ifs.good()||!ifs.is_open()) throw std::runtime_error("Unable to access the given chart file.");
...
void MusicAlbum::parse_line(std::string buf)
{
    ...
    if(std::count_if(buf.begin(),buf.end(),[](char chu){return chu==':';})==0||std::count_if(buf.begin(),buf.end(),[](char chu){return chu==':';})>2) std::runtime_error("The line \""+buf+"\" has problems with the number of colons.");
    ...
    if(*iter==':') iter++;
    else throw std::runtime_error("Wrong input! [at minutes]");
    dataflow>>min_time;
    dataflow.str(""); dataflow.clear();
    for(;iter!=buf.end()&&*iter!='-';iter++)
    {
    ...
    if(*iter=='-') iter++;
    else throw std::runtime_error("Wrong input! [at seconds]");
    dataflow>>sec_time;
